...
import { useSalonStore, PortfolioPhoto, Service } from "@/store/salon";
import { addPortfolioPhotoAction, updatePortfolioPhotoAction, deletePortfolioPhotoAction } from "@/app/actions";
...
export default function ClientPortfolioManager({ tenantId, domain, portfolio, services }: ClientPortfolioManagerProps) {
  const { t } = useI18n();
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      try {
        if (editingPhotoId) {
          await updatePortfolioPhotoAction(tenantId, domain, editingPhotoId, {
          ...
          await addPortfolioPhotoAction(tenantId, domain, {
            id: uuid(),
            imageUrl: form.imageUrl,
            category: form.category,
            caption: form.caption,
      ...
      {(adding || editingPhotoId) && (
        <div className="bg-card border-2 border-primary rounded-3xl p-6 shadow-lg space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleSave} disabled={!form.imageUrl || !form.category || isPending}
      ...
      <ConfirmModal 
        ...
        onConfirm={() => {
          if (deleteConfirm) {
            startTransition(async () => {
              try {
                await deletePortfolioPhotoAction(tenantId, domain, deleteConfirm);
