...
import { Calendar, ExternalLink, Images, ChevronRight, MessageCircle } from "lucide-react";
import Link from "next/link";
import Image from "next/image";

export default function PortfolioPage() {
  const { t } = useI18n();
  const { portfolio, services, settings } = useSalonStore();
  ...
  return (
    <div className="flex-1 w-full bg-background transition-colors duration-300">
      ...
      {/* Filter Bar */}
      {categories.length > 1 && (
        <div className="max-w-5xl mx-auto px-4 mb-8 overflow-x-auto">
        ...
        >
          ...
          >
            <div className="relative w-full h-[60vh]">
              <Image src={selectedPhoto.imageUrl} alt={selectedPhoto.caption || ""} fill sizes="(max-width: 1024px) 100vw, 800px" className="object-cover" />
            </div>
